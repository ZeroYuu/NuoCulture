<template>
            ...
            <el-form :inline="true" :model="searchParam" class="forme" size="default" label-position="right"
                ...
                <el-row>
                    <el-col :span="8" :offset="0">
                        <el-form-item label="标题" filterable>
                            <el-input v-model="searchParam.hp_title" clearable />
                        </el-form-item>
                    </el-col>
                    <el-col :span="8" :offset="0">
                        <el-form-item label="备注">
                            <el-input v-model="searchParam.hp_remark" clearable />
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
    ...
    <div class="table">
        ...
        <el-table ref="tableList" :data="tableData" stripe
            ...
            <el-table-column prop="hp_title" label="标题" :show-overflow-tooltip="true" />
            ...
            <el-table-column label="备注">
                <template #default="scope">
                    {{ scope.row.hp_remark ? scope.row.hp_remark : '--' }}
                </template>
            </el-table-column>
            <el-table-column label="操作" min-width="180" width="220" align="center">
                <template #default="scope">
                    ...
                    <el-button link type="primary" size="small" @click="deleteBtn(scope.row.hp_id)">
                        删除
                    </el-button>
                </template>
            </el-table-column>
            ...
            <el-form ref="addRef" label-width="auto" :model="addModel" :inline="true" :rules="rules">
                <el-row>
                    <el-col :span="12" :offset="0">
                        <el-form-item label="标题" prop="hp_title">
                            <el-input v-model="addModel.hp_title" clearable/>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12" :offset="0">
                        <el-form-item label="备注" prop="hp_remark">
                            <el-input v-model="addModel.hp_remark" clearable/>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12" :offset="0">
                        <el-form-item label="图片" prop="hp_path">
...
import { addApi, getListApi, editApi, deleteApi,allDeleteApi, uploadApi } from '@/api/backend/Culture/Carouse/index'
...
const resetBtn = () => {
    ...
    searchParam.hp_title = '',
        searchParam.hp_remark = '',
        getList();
}
...
const allDeleteBtn = async () => {
    ...
    ElMessageBox.confirm(
        ...
        .then(async () => {//点击确定
            const ids = multipleSelection.value.map((item: { hp_id: any; }) => item.hp_id);
            console.log(ids);
    ...
    addRef.value?.validate(async (valid) => {
        if (valid) {
            ...
            if (MyPicUploadRef.value?.fileList.length) {
                ...
                if (uploadMsg) {
                    addModel.hp_path = uploadMsg.data;
                }
            }
            if (tags.value == "0") {
                //添加
