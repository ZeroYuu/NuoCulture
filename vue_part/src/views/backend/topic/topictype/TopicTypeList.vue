<template>
                ...
                <el-row>
                    <el-col :span="8" :offset="0">
                        <el-form-item label="话题类型" >
                            <SelectChecked :options="nameoptions" @change="selected" ref="selectRef"
                                v-model="searchParam.topic_type_name" filterable style="width: 220px;"/>
                        </el-form-item>
                    </el-col>
                    <el-col :span="8" :offset="0">
                        <el-form-item label="编码">
                            <SelectChecked :options="codeoptions" @change="selected" ref="selectRef"
                                v-model="searchParam.topic_type_encode" filterable style="width: 220px;"/>
                        </el-form-item>
                    </el-col>
                    <el-col :span="8" :offset="0">
                        <el-form-item label="备注">
                            <el-input v-model="searchParam.topic_type_remark" clearable />
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
    ...
    <div class="table">
        ...
        <el-table ref="tableList" :data="tableData" stripe
            ...
            <el-table-column prop="topic_type_name" label="话题名称" />
            <el-table-column label="编码">
                <template #default="scope">
                    {{ scope.row.topic_type_encode ? scope.row.topic_type_encode : "--" }}
                </template>
            </el-table-column>
            <el-table-column label="备注">
                <template #default="scope">
                    {{ scope.row.topic_type_remark ? scope.row.topic_type_remark : "--" }}
                </template>
            </el-table-column>
            <el-table-column label="操作" min-width="180" width="220" align="center">
                <template #default="scope">
                    <el-button link size="small" @click="editBtn(scope.row)">
                        编辑
                    </el-button>
                    <el-button link  size="small" @click="deleteBtn(scope.row.topic_type_id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
            ...
            <el-form ref="addRef" label-width="auto" :model="addModel" :inline="true" :rules="rules">
                <el-row>
                    <el-col :span="12" :offset="0">
                        <el-form-item label="标题" prop="topic_type_name">
                            <el-input v-model="addModel.topic_type_name" clearable/>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12" :offset="0">
                        <el-form-item label="编码" prop="topic_type_encode">
                            <el-input v-model="addModel.topic_type_encode" clearable/>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12" :offset="0">
                        <el-form-item label="备注" prop="topic_type_remark">
                            <el-input v-model="addModel.topic_type_remark" clearable/>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
...
import { addApi, getListApi, editApi, deleteApi, allDeleteApi,uploadApi,getCodeSelectApi,getNameSelectApi} from '@/api/backend/Topic/TopicType/index'
...
//下拉数据 后台获取数据
const getNameSelect = async () => {
    let res = await getNameSelectApi();
    if (res.code == 200) {
...
const getCodeSelect = async () => {
    let res = await getCodeSelectApi();
    if (res.code == 200) {
...
const resetBtn = () => {
    ...
    searchParam.topic_type_encode = '';
    searchParam.topic_type_name= '';
    searchParam.topic_type_remark= '';
    getList();
}
...
const allDeleteBtn = async () => {
    ...
    ElMessageBox.confirm(
        ...
        .then(async () => {//点击确定
            const ids = multipleSelection.value.map((item: { topic_type_id: any; }) => item.topic_type_id);
            console.log(ids);
