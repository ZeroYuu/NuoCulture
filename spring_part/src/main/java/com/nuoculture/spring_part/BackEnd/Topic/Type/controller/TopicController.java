package com.nuoculture.spring_part.BackEnd.Topic.Type.controller;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.nuoculture.spring_part.BackEnd.Topic.Type.entity.TopicTypeInfo;
import com.nuoculture.spring_part.BackEnd.Topic.Type.entity.SelectItem;
import com.nuoculture.spring_part.BackEnd.Topic.Type.entity.TypeParam;
...
public class TopicController {
    ...
    //增加用户
    @PostMapping
    public ResultVo add(@RequestBody TopicTypeInfo topicTypeInfo){
        System.out.println(topicTypeInfo);
        boolean temp = topicService.save(topicTypeInfo);
        if(temp){
...
//        boolean temp = userService.updateById(user);
        boolean temp = topicService.removeById(id);
        if(temp){
    ...
    public ResultVo allDelete(@PathVariable("id") int[] Id) {
        ...
        for (int i = 0; i < Id.length; i++) {
            ...
            temp = topicService.removeById(Id[i]);
        }
    ...
    //修改
    @PutMapping
    public ResultVo edit(@RequestBody TopicTypeInfo topicTypeInfo){
        System.out.println("useEdit");
        System.out.println(topicTypeInfo);
        boolean temp = topicService.updateById(topicTypeInfo);
        if(temp){
    ...
    public ResultVo getList(TypeParam param){
        ...
        //构建分页对象
        IPage<TopicTypeInfo> page=new Page<>();
        page.setCurrent(param.getCurPage());
        page.setSize(param.getPageSize());
        //构造查询条件
        QueryWrapper<TopicTypeInfo> wrapper=new QueryWrapper<>();
        //收货人姓名模糊查询
        if(StringUtils.isNotEmpty(param.getTopic_type_name())){
            wrapper.lambda().like(TopicTypeInfo::getTopic_type_id,param.getTopic_type_name());
        }
        if(StringUtils.isNotEmpty(param.getTopic_type_encode())){
            wrapper.lambda().like(TopicTypeInfo::getTopic_type_id,param.getTopic_type_encode());
        }
        //收货人电话模糊查询
        if(StringUtils.isNotEmpty(param.getTopic_type_remark())){
            wrapper.lambda().like(TopicTypeInfo::getTopic_type_remark,param.getTopic_type_remark());
        }

        
        IPage<TopicTypeInfo> pages = topicService.page(page, wrapper);
    ...
    public ResultVo getCodeSelectList(){
        ...
        Optional.ofNullable(list).orElse(new ArrayList<>())
                .forEach(item->{
                    ...
                    vo.setLabel(item.getTopic_type_encode());
                    selectItems.add(vo);
                });
